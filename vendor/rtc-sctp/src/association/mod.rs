...
impl Association {
    ...
    /// Payload capacity that can leave the pending queue on the next transmit
    /// poll. Unlike stream buffered_amount, this does not impose an artificial
    /// cap on packets already on the wire. Reserve all association pending data
    /// so another stream cannot make this capacity appear available twice.
    pub fn immediate_send_capacity(&self) -> usize {
        let pending = self.pending_queue.get_num_bytes();
        (self.cwnd as usize)
            .saturating_sub(self.inflight_queue.get_num_bytes())
            .min(self.rwnd as usize)
