...
use crate::peer_connection::message::internal::{DTLSMessage, DataChannelEvent, RTCMessageInternal};
use bytes::BytesMut;
    ...
    I: Interceptor,
{
    ...
    /// Application payload bytes awaiting SCTP acknowledgement on this channel,
    /// including writes accepted by the peer but not yet passed to SCTP.
    ///
    /// This deliberately includes in-flight bytes; it is not the W3C unsent-only
    /// `bufferedAmount`. It supports admission control for ephemeral reports.
    /// Only valid for an open channel. No threshold event is needed to sample it.
    pub fn outstanding_payload_bytes(&mut self) -> Result<usize> {
        ...
        let queued: usize = self.peer_connection.pipeline_context.endpoint_handler_context
            .write_outs.iter().filter_map(|msg| match &msg.message {
                RTCMessageInternal::Dtls(DTLSMessage::DataChannel(app))
                    if app.data_channel_id == self.id => match &app.data_channel_event {
                        DataChannelEvent::Message(msg) => Some(msg.data.len().max(1)),
                        _ => None,
                    },
                _ => None,
            }).sum();
        ...
        Ok(queued.saturating_add(association.stream(self.id)?.buffered_amount()?))
    }

    /// Admit a binary message only if queued plus unacknowledged payload stays
    /// within `limit`. A false result queues nothing: callers should resample
    /// current state, rather than retry a stale report. The peer's mutable borrow
    /// makes the measurement and admission atomic with respect to other writes.
    pub fn try_send_with_outstanding_limit(&mut self, data: BytesMut, limit: usize) -> Result<bool> {
        let outstanding = self.outstanding_payload_bytes()?;
        if data.len().max(1) > limit.saturating_sub(outstanding) {
            return Ok(false);
        }
        self.send(data)?;
        Ok(true)
