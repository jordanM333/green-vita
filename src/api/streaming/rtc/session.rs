...
impl<B: RtcSessionBackend> RtcSession<B> {
    ...
    pub fn set_remote_answer(&mut self, sdp: impl Into<String>) -> Result<()> {
        let sdp = sdp.into();
        let payloads = super::feedback::remb_payloads(&sdp);
        let answer = RTCSessionDescription::answer(sdp.into());
        self.peer
            .set_remote_description(answer?)
            .context("failed to set remote rtc answer")?;
        self.remb_payloads = payloads;
        self.remb_active = false;
        Ok(())
    }
    ...
    pub async fn pump(&mut self) -> Result<Vec<RTCIceCandidateInit>> {
        ...
        if self.remb_active && self.last_remb.is_none_or(|last| now.duration_since(last) >= super::feedback::FEEDBACK_INTERVAL) {
            self.last_remb = Some(now);
            match self.video.request_bitrate_ceiling(&mut self.peer) {
                Some(true) => self.remb_queued += 1,
                Some(false) => self.remb_failed += 1,
                None => {}
            }
        }
        ...
        if let Some(status) = self.video.status(now) {
            ...
            let feedback = format!("Video payload:{rate} REMB:{} cap:2000k queued:{} err:{}",
                if self.remb_active { "on" } else { "off" }, self.remb_queued, self.remb_failed);
            ...
            match message {
                RTCMessage::RtpPacket(track_id, packet) => {
                    if self.video.handles(&track_id) {
                        self.remb_active = self.remb_payloads.contains(&packet.header.payload_type);
